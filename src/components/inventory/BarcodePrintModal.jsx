...
function generateZPL(item, quantity = 1) {
...
function BarcodeLabel({ value, itemNameAr, itemNameEn }) {
...
// ─── Main modal ────────────────────────────────────────────────────────────────
export default function BarcodePrintModal({ item, onClose }) {
  const { t } = useLanguage()
  ...
  const handleZebraPrint = async () => {
    ...
    const zpl = generateZPL(item, qty)
  ...
  // ── Debug: send minimal ZPL test label ────────────────────────────────────
  const handleDebugPrint = async () => {
    const testZpl = '^XA^MMT^PW304^LL200^LS0^CF0,30^FO0,80^FB304,1,0,C,0^FDFMAC TEST^FS^XZ'
    try {
      const device = await getZebraDevice()

      // Try A: JSON wrapper
      const r1 = await fetch('http://localhost:9100/write', {
        method: 'POST', mode: 'cors',
        headers: { 'Content-Type': 'text/plain;charset=UTF-8' },
        body: JSON.stringify({ device, data: testZpl }),
      })
      await r1.text()

      // Try B: raw ZPL (always attempt both so we can compare)
      const r2 = await fetch('http://localhost:9100/write', {
        method: 'POST', mode: 'cors',
        headers: { 'Content-Type': 'text/plain' },
        body: testZpl,
      })
      ...
      console.error('[Zebra debug] error:', err)
    }
  }
...
.code { font-size:5pt; text-align:center; line-height:1; }
.name {
          ...
          >
            ...
            <div className="inv-bc-actions">
              ...
              <button
                ...
                title="Send minimal ZPL test — check console for result"
                style={{ fontSize: '0.72rem', opacity: 0.7 }}
              >
                Debug: Test Raw Print
              </button>
